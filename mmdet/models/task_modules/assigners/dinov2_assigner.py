...
class DINOv2Assigner(BaseAssigner):
    ...
    def assign(self,
        ...
        pred_labels = pred_instances.labels  # 需要预测实例的标签
        ...
        for class_idx in range(1, self.num_classes + 1):  # 假设类别标签从1开始
            pred_class_mask = (pred_labels == class_idx)
            gt_class_mask = (gt_labels == class_idx)

            pred_class_instances = pred_instances[pred_class_mask]
            gt_class_instances = gt_instances[gt_class_mask]

            if len(gt_class_instances) == 0:  # 如果该类别没有真实实例
                assigned_gt_inds[pred_class_mask] = 0  # Assign as background
                continue
            ...
            for match_cost in self.match_costs:
                cost = match_cost(
                    pred_instances=pred_class_instances,
                    gt_instances=gt_class_instances,
                    img_meta=img_meta)
            ...
            assigned_gt_inds[pred_class_mask][matched_row_inds] = matched_col_inds + 1
            assigned_labels[pred_class_mask][matched_row_inds] = class_idx
