...
class DINOv2(DINO):
    ...
    def pre_decoder(
        ...
        topk_coords_list = []  # 用于存放每个类别的前60个坐标
        topk_scores_list = []  # 用于存放每个类别的前60个分类置信度
        topk_coords_unact_list = []  # 用于存放每个类别的前60个非激活坐标
        num_topk = self.num_queries // cls_out_features

        for i in range(cls_out_features):
            # 提取特定类别的置信度
            class_scores = enc_outputs_class[:, :, i]

            # 获取该类别置信度最高的前60个索引
            topk_indices = torch.topk(class_scores, k=num_topk, dim=1)[1]

            # 获取对应的坐标
            coords_unact = torch.gather(
                enc_outputs_coord_unact, 1,
                topk_indices.unsqueeze(-1).repeat(1, 1, 4))
            coords = coords_unact.sigmoid()

            # 获取对应的类别得分
            class_score_topk = torch.gather(enc_outputs_class, 1, topk_indices.unsqueeze(-1).repeat(1, 1, cls_out_features))
        ...
        # 将结果堆叠到一个张量中
        topk_coords = torch.cat(topk_coords_list, dim=1)  # [batch_size, self.num_queries, 4]
        topk_score = torch.cat(topk_scores_list, dim=1)  # [batch_size, self.num_queries,15]
        topk_coords_unact = torch.cat(topk_coords_unact_list, dim=1)  # [batch_size, self.num_queries, 4]



        label_index = torch.zeros(bs, self.num_queries)
        # 对于tensor的每个行，每60个元素进行加1
        for i in range(self.num_queries// num_topk):
            label_index[:, i * num_topk:(i + 1) * num_topk] += i
        device=self.label_embeddings.weight.device
        label_index=label_index.to(device).long()
        query = self.label_embeddings(label_index)
