...
class SimMinLoss(nn.Module):
    ...
    def forward(self, data, n):
        ...
        """
        B, M, C = data.size()
        N = M // n  # Assuming M is divisible by n

        # Reshape data to [B, n, N, C]
        data = data.view(B, n, N, C)

        # Compute pairwise cosine similarity for each sample in n excluding self-similarity
        losses = []
        for i in range(n):
            for j in range(n):
                if i != j:
                    sim = self.cos_sim(data[:, i, :, :], data[:, j, :, :])
                    loss = -torch.log(1 - sim)
                    losses.append(loss)

        # Concatenate all computed losses
    ...
    def cos_sim(embedded_fg, embedded_bg):
        ...
        sim = torch.matmul(embedded_fg, embedded_bg.transpose(-2, -1))

        # Clamping the similarity value to avoid numerical issues during log operation
        sim = torch.clamp(sim, min=0.0005, max=0.9995)

        # Create a mask to zero out self-similarities (diagonal elements)
        mask = ~torch.eye(sim.size(-1), dtype=torch.bool, device=sim.device)
        sim = sim * mask.unsqueeze(0)

        return sim.mean(dim=(-2, -1))
#
# # Example usage:
# sim_min_loss = SimMinLoss()
# input_data = torch.randn(10, 20, 30)  # [B, M, C]
# loss = sim_min_loss(input_data, n=5)  # n is the hyperparameter
...
class SimMaxLoss(nn.Module):
    ...
    def forward(self, input_tensor, n):
        B, M, C = input_tensor.size()
        N = M // n
        input_tensor = input_tensor.view(B, n, N, C)

        all_losses = []
        for i in range(B):
            for j in range(n):
                embedded = input_tensor[i, j]
                sim = cos_sim(embedded, embedded)
                loss = -torch.log(sim)
                loss[loss < 0] = 0
