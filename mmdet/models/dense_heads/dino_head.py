...
class DINOHead(DeformableDETRHead):
    ...
    def loss_by_feat(
        ...
        loss_dict = super(DeformableDETRHead, self).loss_by_feat(
            all_layers_matching_cls_scores, all_layers_matching_bbox_preds,
            batch_gt_instances, batch_img_metas, batch_gt_instances_ignore)  # 这里是计算decoder输出的预测使用detr中的loss进行计算match-loss再使用匈牙利匹配6次，TODO：是不是使用匈牙利匹配呢？是的话感觉会很麻烦没法弄固定类别需要改
