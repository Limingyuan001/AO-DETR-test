...
# InfoNCE 类
# done 这个版本是为了避免类内的元素参与计算，使用掩码将相似度矩阵中类内相似度去除掉
class InfoNCE(nn.Module):
    ...
    def forward(self, embedded, class_num, temperature=0.07):
        B, M, C = embedded.size()
        if self.metric == 'l2':
            raise NotImplementedError
        elif self.metric == 'cos':

            # todo infoNCE loss 首先拿到扩展后的cls_weight=classifier,
            #  content_queries=embedded,k=class_num,n=repeat_num
            # 对分类器矩阵扩张到 batch,num_queries,256
            repeat_num = self.num_queries // self.cls_weight.size()[0]
            classifier = torch.repeat_interleave(self.cls_weight, repeats=repeat_num, dim=0).unsqueeze(
                0)  # 复制到queries个数，再扩展B维度
            classifier = torch.repeat_interleave(classifier, repeats=B, dim=0)  # 扩展B通道数量为batch

            # 参数名称转化
            content_queries = embedded
            k = class_num
            n = repeat_num
            # 将输入 reshape 为 [2, k, n, 256]，便于按照类别处理
            classifier = classifier.view(2, k, n, 256)  # [batch_size, k, n, 256]
            content_queries = content_queries.view(2, k, n, 256)  # [batch_size, k, n, 256]

            # 计算正样本对的相似性：相同类别下的样本对齐
            # positive_logits = torch.einsum('bkni,bknj->bknn', classifier, content_queries)  # [batch_size, k, n, n]
            positive_logits = torch.einsum('bknj,bkNj->bknN', classifier, content_queries)  # [2, k, n, n]

            # 计算负样本对的相似性：不同类别的样本不相似
            # 将 classifier 和 content_queries 变形为 [batch_size, k*n, 256]，以便计算跨类别的相似性
            classifier_flat = classifier.view(2, k * n, 256)  # [batch_size, k*n, 256]
            content_queries_flat = content_queries.view(2, k * n, 256)  # [batch_size, k*n, 256]

            # 计算所有样本之间的相似性：classifier 和 content_queries 的全局内积
            all_logits = torch.einsum('bik,bjk->bij', classifier_flat, content_queries_flat)  # [batch_size, k*n, k*n]

            # todo 使用对角线方形，ie 类内，掩码来取消掉类内相似度
            # 应用一个掩码将对角线上的方块的相似度置零，其他位置保留
            mask = self.create_mask(k*n, k).to(all_logits.device)
            mask = mask.unsqueeze(0).expand(B, k*n, k*n)  # 扩展掩码到批次大小
            # 对类内损失进行掩码操作
            neg_logits = all_logits.masked_fill(~mask, 0)  # 应用掩码upsampled_similarity_matrix
            # 提取正样本对的相似度（对角线上元素）：每个类别和自身的相似度
            positive_logits_diag = positive_logits.diagonal(dim1=-2, dim2=-1).unsqueeze(-1)  # [batch_size, k, n, 1]

            # 将正样本对的对角线部分加入到 logits 中并拼接负样本对
            logits = torch.cat([positive_logits_diag.view(2, k, n, 1), neg_logits.view(2, k, n, -1)],
                               dim=-1)  # todo [batch_size, k, n, (k-1)*n+1]

            # Temperature scaling
            logits = logits / temperature

            # 生成标签，正样本对标签为 0 [batch_size, k, n], 由于所有正样本都在第一个位置所以用0来表示，注意这里用的是标签维度比样本维度少一个的形式。
            labels = torch.zeros(2, k, n).to(classifier.device)  # 正样本对标签为 0

            # 计算 InfoNCE loss (cross-entropy)
            loss = F.cross_entropy(logits.view(-1, logits.shape[-1]), labels.view(-1).long())
...
class DINOHeadv4(DeformableDETRHead):
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        # todo infoNCE loss 首先拿到扩展后的cls_weight,content_queries,k,n
        # 初始化infoNCE loss
        InfoNCE_loss = InfoNCE(num_queries=self.num_queries, cls_weight=self.cls_branches[-1].weight)
        info_loss = InfoNCE_loss(queries_batch, class_num, temperature=1/0.07)  # temperature=1 为n-pair loss， =1/0.07 为infoNCE
        losses['info_loss'] = info_loss.sum() / len(indices_list)  # 假设返回的是 (L*B,) 形状的张量
        return losses
