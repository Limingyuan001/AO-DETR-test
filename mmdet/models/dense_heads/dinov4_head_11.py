...
class SimMinLossv2(nn.Module):
    ...
    def forward(self, embedded, class_num):
        ...
        if self.metric == 'l2':
            ...
            normalized_weight = torch.repeat_interleave(normalized_weight, repeats=repeat_num, dim=0).unsqueeze(
                0)  # 复制到queries个数，再扩展B维度
            normalized_weight = torch.repeat_interleave(normalized_weight, repeats=B, dim=0)  # 扩展B通道数量为batch



            similarity_matrix_ww = torch.matmul(normalized_weight, torch.transpose(normalized_weight, 1, 2))
            ...
            margin=torch.exp(similarity_matrix_ww*3-2)
