...
class SimMaxLoss(nn.Module):
    ...
    def forward(self, embedded, class_num):
        ...
        if self.metric == 'l2':
            ...
            # 过滤掉对角线上及负损失值
            # loss = loss[loss > 0.01] # todo 这个应该放在排名加权前，否则可能会出问题

            # todo 应用对角线掩码
            # 创建一个对角线为 True，其他位置为 False 的布尔掩码
            mask_eye = torch.eye(sim.size()[-1], dtype=torch.bool)
            loss[:, mask_eye] = float('nan')
        ...
        if self.reduction == 'mean':
            # return torch.mean(loss)
            return torch.nanmean(loss) # 忽略nan
        elif self.reduction == 'sum':
            return torch.sum(loss)
        else:
            return loss
