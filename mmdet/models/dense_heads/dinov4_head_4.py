...
class SimMinLossv2(nn.Module):
    ...
    def forward(self, embedded, class_num):
        ...
        if self.metric == 'l2':
            ...
            # #  todo A1这里利用相似度矩阵对loss矩阵进行修正；1.得到扩展的相似度矩阵；2.利用相似度矩阵对loss进行正相关惩罚
            # # 计算所有向量之间的余弦相似度
            # # 首先对 weight 矩阵进行 L2 归一化
            ...
            # # Coefficient = torch.exp(upsampled_similarity_matrix)
            # # Coefficient = -torch.log(1 - upsampled_similarity_matrix) #  这个弄错了，不应该有负号
            # # Coefficient = -1/(1-upsampled_similarity_matrix)
            # # Coefficient = torch.exp(2*upsampled_similarity_matrix)
            # # Coefficient = torch.exp(0.5*upsampled_similarity_matrix)
            # # Coefficient = torch.pow(1-sim, -0.25)
            Coefficient = torch.pow(1-sim, -1)
            #
            #
            #
            # # todo A2试试直接用样本相似度进行加权
            # # Coefficient = torch.exp(sim)
            # Coefficient = 1/torch.mul((1-sim), (1-sim))
            # # Coefficient = 1/torch.pow(1-sim, 3)




            loss = torch.mul(loss, Coefficient)
