...
def parse_args():
    ...
    # swin-L pixray DINO
    parser.add_argument('--config', default='../../configs/dino/dino-5scale_swin-l_8xb2-12e_pixray_mine_q900.py',
    ...
    parser.add_argument(
        '--prediction_path',
        default=r'D:\Projects\DINO_mmdet3\mmdetection\tools\PklForConfusion\50/epoch12.pkl',
        help='prediction path where test .pkl result')
    parser.add_argument('--name', default="50_e12.jpg") # TODO 每次得和prediction-path一起对应上
    parser.add_argument(
        '--save_dir',
        default='./ConfusionResult/',
        help='directory where confusion matrix will be saved')
    parser.add_argument(
        '--show', action='store_false', help='show confusion matrix')
    parser.add_argument(
        '--color-theme',
        default='plasma',
...
def plot_confusion_matrix(confusion_matrix,
        ...
        for j in range(num_classes):
            ax.text(
                ...
                '{}%'.format(
                    ...
                    round(confusion_matrix[
                              i,
                              j], 1) if not np.isnan(confusion_matrix[i, j]) else -1),
                ha='center',
                va='center',
                color='w',
                size=7)
    ...
    if save_dir is not None:
        plt.savefig(
            ...
            os.path.join(save_dir, args.name), format = 'jpeg'
        )
    if show:
        plt.show()
